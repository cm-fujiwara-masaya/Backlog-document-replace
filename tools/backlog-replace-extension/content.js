...
(function () {
  ...
  /**
   * execCommand("insertText") 実行後、ProseMirrorがMutationObserverでDOM変更を
   * 検知し内部状態と同期する（再描画される）のを待つ。
   * 全置換で待たずに連続実行すると、ProseMirrorが複数の変更をまとめて解釈しようとして
   * テーブルなど構造の厳密なノードで対応関係が崩れる（表が壊れる）ことがあるため必須。
   */
  ...
  async function autoReplace(searchText, replaceText, caseSensitive) {
    ...
    const result = await replaceAllText(searchText, replaceText, caseSensitive);
   ...
   * ノードまたぎの検索に対応。
   *
   * 重要: 1件置換するたびにProseMirrorのDOM同期(waitForDomSettle)を待ってから
   * 次のマッチをライブDOMから再検索する。以前は事前に集めた全マッチのノード参照を
   * 待機なしで連続してexecCommandに渡していたが、ProseMirrorが複数のDOM変更を
   * まとめて解釈しようとし、テーブルなど構造が厳密なノードで対応関係が崩れて
   * 表のレイアウトが壊れる不具合があった。
   *
   * 置換後文字列に検索文字列が含まれるケース（例: 検索"a"→置換"aa"）で
   * 無限ループにならないよう、事前に数えた件数分だけ処理する。
   */
  async function replaceAllText(searchText, replaceText, caseSensitive) {
    const editor = getEditorElement();
    if (!editor || !searchText) return { success: false, count: 0, error: 'editor_not_found' };
    if (!isEditorEditable())
      return { success: false, count: 0, error: "not_editable" };

    const initialCount = findTextAcrossNodes(searchText, caseSensitive).length;
    // マッチ0件は「成功した上でスキップ」扱い（エラーではない）
    if (initialCount === 0) return { success: true, count: 0 };

    let totalReplaced = 0;
    for (let i = 0; i < initialCount; i++) {
      // 直前の置換でDOM構造が変わっている可能性があるため、都度ライブDOMから再検索する
      const matches = findTextAcrossNodes(searchText, caseSensitive);
      if (matches.length === 0) break; // 想定より早く無くなった場合は打ち切り

      // DOM順で一番後ろのマッチから処理する（先頭側のオフセットへの影響を避けるため）
      const match = matches[matches.length - 1];
      try {
        selectAndReplaceRange(match, replaceText);
        totalReplaced++;
      } catch (e) {
        /* 個別失敗はスキップ（無限ループ防止のためループ自体は続行しない） */
        break;
      }

      // ProseMirrorがDOM変更を検知し内部状態と同期するのを待ってから次へ
